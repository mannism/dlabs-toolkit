...
|---|---|---|---|
| `@diabolicallabs/llm-client` | published | [`packages/llm-client/manifest.yaml`](packages/llm-client/manifest.yaml) | Unified LLM API — Anthropic, OpenAI, Gemini, DeepSeek (impl); Perplexity (stub). Streaming, retry, structured output, token normalization. |
| `@diabolicallabs/agent-sdk` | in-progress | [`packages/agent-sdk/manifest.yaml`](packages/agent-sdk/manifest.yaml) | Cost-tracking middleware wrapping llm-client. Fire-and-forget ingestion to Agent Spend Dashboard. |
