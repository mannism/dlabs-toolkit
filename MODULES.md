...
| `@diabolicallabs/prompt-registry` | published | 0.1.0 | [`packages/prompt-registry/manifest.yaml`](packages/prompt-registry/manifest.yaml) | Versioned LLM prompt lifecycle implementing admin-standard §S7. `seed()`/`get(name, {version\|latest})`/`publish(name, text, meta)`/`history(name)`/`rollback(name, version)` against a `prompt_versions` table (`UNIQUE (prompt_name, prompt_type, version)`). `PostgresPromptStorageAdapter` reference adapter — structural `PgPoolLike` interface (no hard `pg` dependency), parameterized queries only, transactional `insertVersion()`/`activateVersion()`. `loadSeedFilesFromDirectory()` parses repo-file frontmatter `.md` seeds. `maskPromptBody()`/`redactConnectionString()` sensitivity masking. `runPromptEvalGate()` + `prompt-registry-eval-gate` CLI for CI prompt-change gating. Extracted from FitCheckerApp's `prompt_versions` implementation (fleet's cleanest schema per admin-standard) — README carries the literal migration diff. |
...
| llm-pricing new model rows (Claude 5 family, GPT-5.6, Gemini 3.5/3.6) | `llm-pricing` — 9 new model pricing entries: `claude-fable-5`, `claude-opus-5`, `claude-sonnet-5`, `claude-mythos-5`, `gpt-5.6-{sol,terra,luna}` (`hasInvisibleReasoningTokens`), `gemini-3.5-flash-lite`, `gemini-3.6-flash`; `versionedAt` bumped to `2026-07-25` | ✅ shipped 2026-07-29 (PR #218) |
| llm-client@6.3.0 — reasoning-effort passthrough | `llm-client@6.3.0` (minor) — `reasoningEffort` on `LlmCallOptions` (Anthropic/OpenAI/Gemini; Perplexity/DeepSeek reject with `bad_request`), `LlmUsage.reasoningTokens`, `reasoningEffort` capability dialect tag, new `claude-opus-5`/`gpt-5.6-{sol,terra,luna}` capability rows. Unblocks the Labs Effort-Level A/B Harness. | ✅ shipped 2026-07-29 (PR #225, #226) |
| Changesets bot-PR approval fix | `release.yml` — `changesets/action` now authenticates via `secrets.CHANGESETS_PAT` (fine-grained PAT) instead of the default `GITHUB_TOKEN`, exempting the auto-created "Version Packages" PR from GitHub's 2026-06 bot-PR workflow-approval gate. dlabs-toolkit-specific (only repo using Changesets' PR-based release flow; every other fleet repo uses semantic-release, which pushes directly to `main`). | ✅ shipped 2026-07-29 (PR #227) |
