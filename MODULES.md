...
| llm-client@6.3.0 — reasoning-effort passthrough | `llm-client@6.3.0` (minor) — `reasoningEffort` on `LlmCallOptions` (Anthropic/OpenAI/Gemini; Perplexity/DeepSeek reject with `bad_request`), `LlmUsage.reasoningTokens`, `reasoningEffort` capability dialect tag, new `claude-opus-5`/`gpt-5.6-{sol,terra,luna}` capability rows. Unblocks the Labs Effort-Level A/B Harness. | 🟡 PR open 2026-07-29 (feat/llm-client-reasoning-effort-passthrough) |
