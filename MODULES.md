...
|---|---|---|---|---|
| `@diabolicallabs/llm-client` | published | 0.1.1 | [`packages/llm-client/manifest.yaml`](packages/llm-client/manifest.yaml) | Unified LLM API — Anthropic, OpenAI, Gemini, DeepSeek (impl); Perplexity (stub). Streaming, retry, structured output, token normalization. |
...
| Week 5 | `@diabolicallabs/notion` + `@diabolicallabs/rate-limiter` (parallel) | not yet briefed |
| Week 6+ | GEOAudit pilot migration; Trusted Publishing migration on npm | scoped, not briefed |
| Deferred | Perplexity provider — add when a consumer needs it (no flag mechanism) | indefinite |
