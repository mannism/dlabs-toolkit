...
- Removed the stale header note claiming `deepseek-v4-pro` has an active promotional discount — that promo expired 2026-05-31 and the table price already reflects the no-promo rate.
- Rewrote the file header: it previously claimed `src/table.ts` was auto-generated from a `pricing/table.json` + `pricing/sync-bundled.mjs` pipeline that does not exist anywhere in this repo. The header now states the truth — `src/table.ts` IS the source of truth, hand-maintained directly.
