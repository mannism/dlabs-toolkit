...
- `gemini` provider: full implementation using `@google/genai` SDK v1.x. Supports `complete()`, `stream()`, and `structured()`. System instructions via `config.systemInstruction`. Token normalisation via `usageMetadata`. Error normalisation via the publicly-exported `ApiError` class (status always number). Network errors handled by `normaliseThrownError`.
...
- Tests: `gemini.test.ts` (20 tests), `deepseek.test.ts` (22 tests), `error-normalise.test.ts` extended with Gemini + DeepSeek normalisation tests (22 total). `client.test.ts` updated to mock all four providers and test only Perplexity as stub.
