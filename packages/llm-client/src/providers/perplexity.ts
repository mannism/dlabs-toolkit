...
export function createPerplexityProvider(config: LlmClientConfig): LlmClient {
  ...
  /**
   * Perplexity does not support a Files API.
  ...
  const files: LlmFilesApi = {
    async upload(): Promise<never> {
      throw new LlmError({
        message:
          "[llm-client] Provider 'perplexity' does not support Files API. Use inline base64 content instead.",
        provider: PROVIDER,
        kind: 'bad_request',
        retryable: false,
      });
    ...
    async refresh(): Promise<never> {
      throw new LlmError({
        message: "[llm-client] Provider 'perplexity' does not support Files API.",
        provider: PROVIDER,
        kind: 'bad_request',
        retryable: false,
      });
