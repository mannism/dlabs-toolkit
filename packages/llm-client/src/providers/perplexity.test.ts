...
// ─── Multimodal content blocks (v4.2.0) — Option B: defer + reject ────────────

describe('Perplexity provider — multimodal content blocks (v4.2.0)', () => {
  let mockCreate: MockInstance;
  ...
  it('error message names provider and block type', async () => {
    ...
    try {
      ...
      const { LlmError: LE } = await import('../types.js');
      expect(err).toBeInstanceOf(LE);
      const e = err as InstanceType<typeof LE>;
      expect(e.message).toContain("Provider 'perplexity'");
